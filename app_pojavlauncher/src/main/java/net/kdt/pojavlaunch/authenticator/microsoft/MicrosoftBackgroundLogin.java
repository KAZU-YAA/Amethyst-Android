...
public class MicrosoftBackgroundLogin {
    ...
    private void fetchOwnedItems(String mcAccessToken) throws IOException {
        ...
        // We don't need any data from this request, it just needs to happen in order
        // for
    ...
    private void checkMcProfile(String mcAccessToken) throws IOException, JSONException {
        ...
        if (conn.getResponseCode() >= 200 && conn.getResponseCode() < 300) {
            ...
            doesOwnGame = false;
            throw new PresentedException(new RuntimeException(conn.getResponseMessage()),
    ...
    /**
     * Set common properties for the connection. Given that all requests are POST,
     * interactivity is always enabled
     */
    private static void setCommonProperties(HttpURLConnection conn, String formData) {
        ...
        try {
            conn.setRequestProperty("Content-Length",
                    Integer.toString(formData.getBytes(StandardCharsets.UTF_8).length));
            conn.setRequestMethod("POST");
